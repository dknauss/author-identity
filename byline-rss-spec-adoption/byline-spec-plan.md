...
Byline (bylinespec.org) is an open specification (v0.1.0 draft, CC0 licensed; namespace `bylinespec.org/1.0`) that extends RSS 2.0, Atom, and JSON Feed with structured author identity, context, and content perspective metadata. It was created by Terry Godier to address "content collapse" — the loss of context when diverse content sources arrive in a unified feed reader stream.
...
- **`byline:role`**: author's relationship to the content (creator, editor, guest, staff, founder, contributor, bot).
...
Current status (March 2026): 5 GitHub stars, 4 commits, zero implementations, zero feed reader support. Very early stage.
...
Being the first CMS plugin to implement the Byline spec would give this fork influence over how the spec evolves. The spec is CC0 licensed and at v0.1.0 draft — early implementors always shape specifications.
