...
For a side-by-side comparison of existing multi-author plugin implementations — the systems this vision builds on — see [multi-author-matrix.md](../research/multi-author-matrix.md). For how the protocols referenced throughout this document cover (or fail to cover) each identity signal across output channels, see [protocol-coverage-map.md](../research/protocol-coverage-map.md).
